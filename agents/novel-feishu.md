...
**重要：当用户说"帮我配置飞书"时，必须按以下步骤执行！**

### 配置位置（全局）
配置保存在以下位置，任何对话都会自动加载：
- 主配置：`~/.opencode/skills/novel/feishu.json`
- 环境变量：`~/.opencode/.env`
...
2. 读取配置文件：`~/.opencode/skills/novel/feishu.json`

3. 将用户提供的信息写入配置文件

4. 同时保存到环境变量文件：`~/.opencode/.env`

5. 告诉用户配置完成，配置会在任何对话中自动加载
